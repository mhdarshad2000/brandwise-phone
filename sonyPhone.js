...
const brandUrl = "https://www.service-center-locator.com/sony-phones/sony-phones-service-center.htm"
    ...
    return new Promise(async (resolve) => {
        try {
            ...
            $(postDiv).find(" div:nth-child(11),div:nth-child(12)").children("ul").map((i, state) => {
                sonyPhone[i] = {}
                sonyPhone[i]['state'] = ($(state).children("strong").text())
                sonyPhone[i]['states'] = []
                $(state).children("li").each(async (j, city) => {
                    sonyPhone[i]['states'][j] = {}
                    sonyPhone[i]['states'][j]['name'] = $(city).text()
                    const link = $(city).children("a").attr("href").replace("../", baseUrl)
                    sonyPhone[i]['states'][j]['link'] = link

                    sonyPhone[i]['states'][j]['city'] = await detailsPage(link, $(city).text())
                })

            })
            setTimeout(() => {
                const brand = JSON.stringify(sonyPhone)
                fs.writeFileSync("./sonyPhone.json", brand)
